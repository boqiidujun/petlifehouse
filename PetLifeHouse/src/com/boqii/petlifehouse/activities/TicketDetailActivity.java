...
public class TicketDetailActivity extends BaseLocationActivity implements OnClickListener, OnCheckedChangeListener, Callback, OnScrollListener {
	...
	protected void initTicketDetail(Ticket ticket) {
		...
		// 悬浮层赋值
		RelativeLayout to_ticket_info = (RelativeLayout) findViewById(R.id.top_ticket_status);
		...
		String format = "0";
		if (distance > 1000) {
			format = String.format(getString(R.string.distance_km), Util.GetDistanceToKM(distance));
		} else {
			format = String.format(getString(R.string.distance_m), distance);
		}
