...
public class NearTicketFragment extends BaseFragment implements
	...
	private void initData(final String KeyWord, final int SortTypeId,
			...
			final int StartIndex, final int Number) {
			...
			if (ticket != null) {
				...
				if(distance>1000){
					format = String.format(
							...
							Util.GetDistanceToKM(distance));
				}else{
					format = String.format(getString(R.string.distance_m),
							distance);
				}
	...
	private MyLocationListener startLListener = new MyLocationListener() {
		...
		public void Success(double lat, double lng) {
			latT = lat;
			lngT = lng;
			initData("", 0, 1, lat, lng, startIndex, 10);
		}
	...
	public void categoryCallback(String name, int typeId) {
		...
		setTitelColor(lst, 0);
		initData("", this.typeId, this.orderId, latT, lngT, 0, 10);
	}
	...
	public void onLoadMore() {//上拉到底加载更多	
		initData("", this.typeId, this.orderId, latT, lngT, startIndex, 10);
	}
